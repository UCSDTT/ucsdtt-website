const slideData = [
  {
    name: 'lifelong friends',
    image: require('./images/alumni.webp'),
    smImage: require('./images/alumni-sm.webp'),
    xsImage: require('./images/alumni-xs.webp'),
    safari: require('./images/alumni.jpg'),
    smSafari: require('./images/alumni-xs.jpg'),
    xsSafari: require('./images/alumni-xs.jpg')
  },
  {
    name: 'competitors',
    image: require('./images/BubbleSoccer.webp'),
    smImage: require('./images/BubbleSoccer-sm.webp'),
    xsImage: require('./images/BubbleSoccer-xs.webp'),
    safari: require('./images/BubbleSoccer.jpg'),
    smSafari: require('./images/BubbleSoccer-xs.jpg'),
    xsSafari: require('./images/BubbleSoccer-xs.jpg')
  },
  {
    name: 'adventurers',
    image: require('./images/cave.webp'),
    smImage: require('./images/cave-sm.webp'),
    xsImage: require('./images/cave-xs.webp'),
    safari: require('./images/cave.jpg'),
    smSafari: require('./images/cave-xs.jpg'),
    xsSafari: require('./images/cave-xs.jpg')
  }
];

const quoteData = [
  {
    text:
      'My favorite part about Theta Tau was when we went to Geisel with literally every person in the fraternity. Wow.',
    active: 'Rishi Dhanaraj',
    activeImage: './images/rishi.webp',
    major: 'Computer Science',
    class: 'Xi Class'
  },
  {
    text: 'I love Theta Tau. I love Theta Tau. I love Theta Tau.',
    active: 'Brian Chan',
    activeImage: './images/bchan.webp',
    major: 'Computer Science',
    class: 'Omicron Class'
  },
  {
    text:
      'My favorite part about Theta Tau was when we went to Geisel with literally every person in the fraternity. Wow.',
    active: 'Rishi Dhanaraj',
    activeImage: './images/rishi.webp',
    major: 'Computer Science',
    class: 'Xi Class'
  }
];

export { slideData, quoteData };
