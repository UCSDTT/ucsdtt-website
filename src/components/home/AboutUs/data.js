const gearPosition = ['top left', 'top right', 'bottom left', 'bottom right'];

const pillarData = [
  {
    info: 'Bonding',
    description: 'Brothers playing basketball at Interchapter Sports Day',
    name: 'Brotherhood',
    content: 'We strive to make the group as closely knit as possible throughout one’s college years. Our goal is to develop bonds between our brothers that will last a lifetime. We foster an inviting, safe, and social environment in which our members become lifelong friends.',
    image: require('./images/brotherhood.webp'),
    safari: require('./images/brotherhood.jpg'),
  },
  {
    info: 'Internship',
    description: 'Brother Tyler Ly after a successful internship with Google',
    name: 'Professionalism',
    content: 'We also seek to shape up one’s professional interests to prepare them for their life ahead of college. We plan various networking opportunities as well as events that provide a sneak peek at different professional careers.',
    image: require('./images/professionalism.webp'),
    safari: require('./images/professionalism.jpg'),
  },
  {
    info: 'Charity',
    description: 'Collecting and donating toys to Syrian refugee families',
    name: 'Service',
    content: 'For the purpose of our members’ personal growth, as well as our community’s benefits, we are involved in projects that give back to the community around us. Serving alongside brothers will establish strong bonds and also guarantee a truly meaningful time through one’s college years.',
    image: require('./images/service.webp'),
    safari: require('./images/service.jpg'),
  },
]

export {gearPosition, pillarData};
